...
Future<Response> _onGetRequest(RequestContext context, String placeId) async {
  ...
  final restaurantView = await db.dbResturants.query(
...
Future<Response> _onPostRequest(RequestContext context, String placeId) async {
  ...
  final insert = DbResturantInsertRequest(
    placeId: placeId,
    name: name,
  ...
  await db.dbResturants.insertOne(insert);
...
Future<Response> _onPutRequest(RequestContext context, String placeId) async {
  ...
  final update = DbResturantUpdateRequest(
    placeId: placeId,
    name: name,
    businessStatus: 'Restaurant',
    tags:
  ...
  await db.dbResturants.updateOne(update);
  return Response(statusCode: HttpStatus.created);
}
...
Future<Response> _onDeleteRequest(
  ...
  await db.dbResturants.deleteOne(placeId);
  return Response(statusCode: HttpStatus.created);
}
