...
class _DBMenuRepository extends BaseRepository
  ...
  Future<List<int>> insert(List<DBMenuInsertRequest> requests) async {
    ...
    var rows = await db.execute(
      Sql.named('INSERT INTO "Menu" ( "category", "resturant_place_id" )\n'
          'VALUES ${requests.map((r) => '( ${values.add(r.category)}:text, ${values.add(r.resturantPlaceId)}:text )').join(', ')}\n'
          'RETURNING "id"'),
  ...
  Future<void> update(List<DBMenuUpdateRequest> requests) async {
    ...
    await db.execute(
      Sql.named('UPDATE "Menu"\n'
          'SET "category" = COALESCE(UPDATED."category", "Menu"."category"), "resturant_place_id" = COALESCE(UPDATED."resturant_place_id", "Menu"."resturant_place_id")\n'
          'FROM ( VALUES ${requests.map((r) => '( ${values.add(r.id)}:int8::int8, ${values.add(r.category)}:text::text, ${values.add(r.resturantPlaceId)}:text::text )').join(', ')} )\n'
          'AS UPDATED("id", "category", "resturant_place_id")\n'
...
class DBMenuInsertRequest {
  DBMenuInsertRequest({
    ...
    this.resturantPlaceId,
  });

  final String category;
  final String? resturantPlaceId;
}
