...
import 'package:user_repository/src/model/model.dart';
...
class UserRepository {
  ...
  Stream<Location> currentLocation() => _userStorage.userLocation();

  /// Fetches user location value from User Storage.
  Location fetchCurrentLocation() => _userStorage.getUserLocation();

  /// Clears user location value in User Storage.
  Future<void> clearCurrentLocation() => _userStorage.clearUserLocation();
  ...
  Future<void> changeLocation({required Location location}) async {
    try {
      await _userStorage.setUserLocation(location: location);
    } catch (error, stackTrace) {
