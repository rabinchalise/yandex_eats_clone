...
class _PaymentProcessModalViewState extends State<PaymentProcessModalView> {
  ...
  Future<void> _processOrder() async {
    ...
    try {
      ...
      context
        ..pop()
        ..pop()
        ..goNamed(AppRoutes.resturants.name);
    } catch (error) {
      _stopwatch.stop();
      _timer?.cancel();
      context
  ...
  Future<void> _handlePayProcess() async {
    final selectCard = context.read<SelectedCardCubit>().state.selectedCard;
    final card = CardDetails(
      number: selectCard.number,
      cvc: selectCard.cvv,
      expirationMonth: int.parse(selectCard.expiryDate.split('/').first),
      expirationYear: int.parse(selectCard.expiryDate.split('/').last),
    );
